...
max_strings = 500 # maximum number of Pauli strings to test
...
use_rotations = False # implement random rotations for CNOT-only depolarization (WARNING: only works with certain circuits)

coherent_error = False # add coherent rx rotation error in addition to incoherent error
twirling = False # implements both Pauli twirling and readout twirling
num_twirls = 250 # how many different circuits to create during Pauli twirling
...
coherent_rotation = 0.15 # amount of rotation error (coherent_error only)
...
def generate_benchmarks():
    ...
    cnot_depo3 = np.zeros(num_strings)
    cnot_depo5 = np.zeros(num_strings)
    ...
    if pauli_sharing == 'full':
        for j in range(benchmark_size): # average depolarization magnitude across many circuits
            ...
            cnot_depo_circ = transpile_circ(filter_2qbt(logical_circ))
            cnot_depo_circ3 = transpile_circ(filter_2qbt(pm3.run(logical_circ)))
            cnot_depo_circ5 = transpile_circ(filter_2qbt(pm5.run(logical_circ)))
            cnot_depo[0] += 1 - run_circ(cnot_depo_circ, benchmark_shots/benchmark_size, measured_qubits)
            cnot_depo3[0] += 1 - run_circ(cnot_depo_circ3, benchmark_shots/benchmark_size, measured_qubits)
            cnot_depo5[0] += 1 - run_circ(cnot_depo_circ5, benchmark_shots/benchmark_size, measured_qubits)
        rida_depo[0] /= benchmark_size
        cnot_depo[0] /= benchmark_size
        cnot_depo3[0] /= benchmark_size
        cnot_depo5[0] /= benchmark_size
        rida_depo.fill(rida_depo[0])
        cnot_depo.fill(cnot_depo[0])
        cnot_depo3.fill(cnot_depo3[0])
        cnot_depo5.fill(cnot_depo5[0])
    ...
    # use the same depolarization estimate for circuits with the same qubits with non-identity operators
        ...
        sim_cnot3 = {} # CNOT-only depolarization for each Pauli string and 3x CNOT gates
        sim_cnot5 = {} # CNOT-only depolarization for each Pauli string and 5x CNOT gates
        ...
        for pstring in sim_paulis:
            ...
            for j in range(benchmark_size): # average depolarization magnitude across many circuits
                ...
                sim_cnot3[pstring] += 1 - run_circ(cnot_depo_circ3, benchmark_shots/benchmark_size, measured_qubits)
                sim_cnot5[pstring] += 1 - run_circ(cnot_depo_circ5, benchmark_shots/benchmark_size, measured_qubits)
            sim_rida[pstring] /= benchmark_size
            sim_cnot[pstring] /= benchmark_size
            sim_cnot3[pstring] /= benchmark_size
            sim_cnot5[pstring] /= benchmark_size
            if progress_flags:
                print('Pauli ', index+1, '/', len(sim_paulis), ' completed')
        ...
        for i in range(num_strings):
            ...
            cnot_depo3[i] = sim_cnot3[frame]
            cnot_depo5[i] = sim_cnot5[frame]
    ...
    # create a seperate depolarization estimate for each Pauli string
        ...
        for i in range(num_strings):
            ...
            for j in range(benchmark_size): # average depolarization magnitude across many circuits
                ...
                cnot_depo_circ = filter_2qbt(circ_t)
                cnot_depo_circ3 = filter_2qbt(circ_t)
                cnot_depo_circ5 = filter_2qbt(circ_t)
                cnot_depo[i] += 1 - run_circ(cnot_depo_circ, benchmark_shots/benchmark_size, measured_qubits)
                cnot_depo3[i] += 1 - run_circ(cnot_depo_circ3, benchmark_shots/benchmark_size, measured_qubits)
                cnot_depo5[i] += 1 - run_circ(cnot_depo_circ5, benchmark_shots/benchmark_size, measured_qubits)
            rida_depo[i] /= benchmark_size
            cnot_depo[i] /= benchmark_size
            cnot_depo3[i] /= benchmark_size
            cnot_depo5[i] /= benchmark_size
            if progress_flags:
    ...
    return rida_depo, cnot_depo, cnot_depo3, cnot_depo5
...
def run_trials():
    ...
    for j in range(num_strings):
        ...
        for i in range(len(shots)):
            ...
            zne[i][j] = e_extrapolate(p1s[0], p1s[1], p1s[2])/(1-trex[j])
        
            p1s[0] = (p1s[0])/(1 - cnot_depo[j]) # perform depolarizing approximation on 1x error extrapolation point
            p1s[1] = (p1s[1])/(1 - cnot_depo3[j]) # perform depolarizing approximation on 3x error extrapolation point
            p1s[2] = (p1s[2])/(1 - cnot_depo5[j]) # perform depolarizing approximation on 5x error extrapolation point
...
rida_depo, cnot_depo, cnot_depo3, cnot_depo5 = generate_benchmarks()
mid = time.time()
if progress_flags:
...
if save_results:
    save_dict = {'nqubits': nqubits, 'error_multiplier': error_multiplier, 
                 'pauli_sharing': pauli_sharing, 'shots': shots, 'layers': layers,
                 'twirling': twirling, 'use_rotations': use_rotations,
