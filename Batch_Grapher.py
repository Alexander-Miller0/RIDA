...
multipliers = True # whether to RMSE as a function of error multiplier
...
paulis = False # whether to graph error as a function of true expectations of Pauli strings
compress = True # whether to graph a subset of graphs for multiplier and shot graphs
...
cnot_color = '#91BE8D'
rida_color = '#FF4400'
border_color = '#3F3F3F' # borders and ticks are slightly faded
...
def graph_shots(dict, ax):
    ...
    ax.set_title(str(dict['nqubits']) + ' qubits, ' + str(dict['error_multiplier']) + 'x error', fontsize=8+8*compress)
    ax.set_xscale('log')
    ax.set_yscale('log')

    ax.set_xlim(dict['shots'][0], dict['shots'][-1]) # cut blank edges off graph
    ax.set_ylim(lower_bound, upper_bound) # create consistent bounds across subplots
...
def graph_predictions(dict, ax):
    ...
    ax.plot(dict['shots'], [1/(np.sqrt(k)*np.mean(1-dict['rida_depo'])) for k in dict['shots']], linewidth=1, color=rida_color, label='RIDA Shot Prediction', zorder=4, linestyle='dashed')
    ax.plot(dict['shots'], [zslope/(np.sqrt(k/3)) for k in dict['shots']], linewidth=1, color=zne_color, label='Exponential ZNE Shot Prediction', zorder=2, linestyle='dashed')
    ax.set_title(str(dict['nqubits']) + ' qubits, ' + str(dict['error_multiplier']) + 'x error', fontsize=8)
    ax.set_xscale('log')
    ax.set_yscale('log')

    ax.set_xlim(dict['shots'][0], dict['shots'][-1]) # cut blank edges off graph
    ax.set_ylim(lower_pred_bound, upper_pred_bound) # create consistent bounds across subplots, no need to include predictions that go past bounds
...
def graph_error_multiplier(dict, ax):
    ...
    ax.set_title(str(dict[0]['nqubits']) + ' qubits', fontsize=10+2*compress)
    ax.set_xlim(error_multipliers[0], error_multipliers[-1]) # cut blank edges off graph
    ax.set_ylim(bottom=0) # lower bound of 0 since plot is not logarithmic
...
def graph_depo(dict, ax):
    ...
    ax.plot([-1, 1], [np.mean(dict['cnot_depo'])-1, 1-np.mean(dict['cnot_depo'])], color=cnot_color, label='CNOT-Only Depolarizing Model', linewidth=0.6)
...
def graph_depo_summary(dict, ax):
    ...
    ax.plot(error_multipliers, true_depos, linewidth=1, color='black', label='Optimal', linestyle='dashed', zorder=1)
    ax.plot(error_multipliers, rida_depos, linewidth=1, color=rida_color, label='RIDA', zorder=0)
    ax.plot(error_multipliers, cnot_depos, linewidth=1, color=cnot_color, label='CNOT-Only Depolarization', zorder=0)
...
# graph error as a function of true expectations of Pauli strings
def graph_pauli(dict, ax):
    ...
    height_cutoff = 1.2 # y limit of the graph - some outliers may exceed this

    # deal with outliers by placing at the y limit with text label of true value
    ...
    for i in range(len(zne_error)): # exponential zne is the only method with outliers due to unstable extrapolation
        if zne_error[i] > height_cutoff - 0.02: # determine if the point is an outlier
            actual_zne.append([dict['statevector'][i], round(zne_error[i], 1), 1]) # append error free expectation, zne value, and layer of the label
            actual_zne[-1][0] = max(min(0.965, actual_zne[-1][0]), -0.965) # prevent label from overlapping the edge
            zne_error[i] = height_cutoff - 0.02 # move original point down to the height limit
    ...
    for i in range(len(actual_zne)): # print labels with the actual extrapolation values under each point
        ax.annotate(str(actual_zne[i][1]), xy=(actual_zne[i][0], height_cutoff - 0.02), xytext=(actual_zne[i][0], height_cutoff - 0.05 - 0.05*actual_zne[i][2]), ha='center', va='bottom', fontsize=5)
    ...
    ax.set_xlim(-1, 1) # true expectations range from -1 to 1
    ax.set_ylim(0, height_cutoff) # cut off height to avoid being distorted by outliers
    ax.tick_params(axis='both', labelsize=7, color=border_color)
    for spine in ax.spines.values():
        spine.set_edgecolor(border_color)
...
lower_pred_bound = 1
upper_pred_bound = 0
upper_ds_bound = 0 # upper bound for depolarizing factor summary graph
for q in range(len(file_name)): # iterate over different numbers of qubits
    for i in range(len(file_name[q])): # iterate over different error multipliers
        ...
        # update bounds if any rmses exceed the bounds
        upper_bound = max( upper_bound, np.max(dict[q][i]['raw_rmse']), np.max(dict[q][i]['rida_rmse']), np.max(dict[q][i]['cnot_rmse']), np.max(dict[q][i]['zne_rmse']))
        lower_bound = min( lower_bound, np.min(dict[q][i]['raw_rmse']), np.min(dict[q][i]['rida_rmse']), np.min(dict[q][i]['cnot_rmse']), np.min(dict[q][i]['zne_rmse']))
        ...
        zslope.append( (uu+uu**(1/2))**(-2)*(0.5*uu**(-1/2)+1) )
        zslope = np.sqrt(zslope[0]**2+zslope[1]**2+zslope[2]**2)
        lower_pred_bound = min( lower_bound, lower_pred_bound, 1/(np.sqrt(dict[q][i]['shots'][-1])*np.mean(1-dict[q][i]['rida_depo'])))
        upper_pred_bound = max( upper_bound, upper_pred_bound, zslope/np.sqrt(dict[q][i]['shots'][0]/3))
...
lower_bound *= 0.9 # make it easier to see the bottom of the graph
...
if shot_graph:
    if compress: # only graph smallest and largest numbers of qubits and error multipliers
        ...
        fig.legend(handles, labels, loc='outside upper center', ncol=2, fontsize=16)
        fig.supxlabel('Shots', fontsize=20)
        fig.supylabel('RMSE', fontsize=20)
    else:
        fig, axs = plt.subplots(nrows=len(file_name), ncols=len(file_name[0]), figsize=(10, 6), constrained_layout=True) # create overarching figure
...
if multipliers:
    if compress: # only graph largest number of qubits
        ...
        fig.legend(handles, labels, loc='outside upper center', ncol=3, fontsize=10.5)
        ...
        fig, axs = plt.subplots(ncols=4, figsize=(10, 2), constrained_layout=True) # create overarching figure
        for i in range(len(file_name)):
            graph_error_multiplier(dict[i], axs[i]) # graph each subplot
        handles, labels = axs[0].get_legend_handles_labels()
        fig.legend(handles, labels, loc='outside upper center', ncol=5, fontsize=8)
...
if depolarization:
    ...
    fig.supxlabel('True Expectation')
    fig.supylabel('Unmitigated Value')
    handles, labels = axs[0][0].get_legend_handles_labels()
    fig.legend(handles, labels, loc='outside upper center', ncol=4, fontsize=8, markerscale=5) # place the legend below the title
    if save_results:
