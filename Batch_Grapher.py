...
def graph_predictions(dict, ax):
    ...
    ax.plot(dict['shots'], [zslope/(np.sqrt(k/3)) for k in dict['shots']], linewidth=1, color=zne_color, label='Exponential ZNE Shot Prediction', zorder=2, linestyle='dashed') # shots split among 3 circuits
...
def graph_pauli(dict, ax):
    ...
    for i in range(len(zne_error)): # exponential zne is the only method with outliers due to unstable extrapolation
        if zne_error[i] > height_cutoff - 0.02: # determine if the point is an outlier
            ...
            actual_zne[-1][0] = max(min(0.93, actual_zne[-1][0]), -0.93) # prevent label from overlapping the edge
    ...
    for i in range(len(actual_zne)): # print labels with the actual extrapolation values under each point
        ax.annotate(str(actual_zne[i][1]), xy=(actual_zne[i][0], height_cutoff - 0.02), xytext=(actual_zne[i][0], height_cutoff - 0.06 - 0.08*actual_zne[i][2]), ha='center', va='bottom', fontsize=6)
...
if paulis:
    ...
    fig.supylabel('Error')
    handles, labels = axs[0][0].get_legend_handles_labels()
    fig.legend(handles, labels, loc='outside upper center', ncol=4, fontsize=11, markerscale=5, frameon=False) # place the legend below the title
    if save_results:
