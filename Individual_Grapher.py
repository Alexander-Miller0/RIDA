...
def graph_shots():
    ...
    if use_rotations:
        fig.get_layout_engine().set(rect=(0, 0, 1, 0.85)) # add additional space at the top to fit the legend and title
        fig.legend(loc='outside upper center', bbox_to_anchor=(0.5, 0.95), ncol=2, fontsize=10) # place the legend below the title
    else:
        fig.get_layout_engine().set(rect=(0, 0, 1, 0.90)) # add additional space at the top to fit the legend and title
        fig.legend(loc='outside upper center', bbox_to_anchor=(0.5, 0.96), ncol=2, fontsize=10) # place the legend below the title
    fig.suptitle(str(nqubits) + ' qubits, ' + error_statement, fontsize=14)
    if save_results:
        if use_rotations:
            file_path = 'ROTATIONS GRAPH - ' + str(nqubits) + ' qubits, ' + error_statement
        else:
            file_path = 'SHOTS GRAPH - ' + str(nqubits) + ' qubits, ' + error_statement
...
def graph_predictions():
    ...
    plt.plot(shots, [1/(np.sqrt(k)*np.mean(1-rida_depo)) for k in shots], linewidth=1, color=rida_color, label='RIDA Shot Prediction', zorder=4, linestyle='dashed')
    plt.plot(shots, [zslope/(np.sqrt(k/3)) for k in shots], linewidth=1, color=zne_color, label='Exponential ZNE\nShot Prediction', zorder=2, linestyle='dashed')
    
    plt.xlabel('Shots')
    plt.ylabel('RMSE')
    fig.get_layout_engine().set(rect=(0, 0, 1, 0.88)) # add additional space at the top to fit the legend and title
    fig.suptitle(str(nqubits) + ' qubits, ' + error_statement, fontsize=14)
    fig.legend(loc='outside upper center', bbox_to_anchor=(0.5, 0.96), ncol=3, fontsize=10) # place the legend below the title
    if save_results:
        file_path = 'PREDICTIONS GRAPH - ' + str(nqubits) + ' qubits, ' + error_statement
        if os.path.exists(file_path + '.png'): # check if the file already exists
            file_path = file_path + str(time.time()) # add a timestamp to avoid overwriting existing file
        plt.savefig(file_path + '.png', dpi=500)
    if print_results:
...
def graph_depo():
    ...
    plt.scatter(statevector, raw[-1], color=raw_color, s=5, alpha=0.5, label='Unmitigated Data')
    ...
    plt.title(str(nqubits) + ' qubits, ' + error_statement, fontsize=14)
    if save_results:
        file_path = 'DEPOLARIZATION GRAPH - ' + str(nqubits) + ' qubits, ' + error_statement
        if os.path.exists(file_path + '.png'): # check if the file already exists
            file_path = file_path + str(time.time()) # add a timestamp to avoid overwriting existing file
        plt.savefig(file_path + '.png', dpi=500)
    if print_results:
...
def graph_depo_bar():
    ...
    plt.xlabel('Method')
    plt.ylabel('Depolarizing Estimate p')
    plt.title(str(nqubits) + ' qubits, ' + error_statement, fontsize=14)
    plt.legend(loc='upper left', fontsize=10, framealpha=1)
    if save_results:
        file_path = 'BAR DEPOLARIZATION GRAPH - ' + str(nqubits) + ' qubits, ' + error_statement
        if os.path.exists(file_path + '.png'): # check if the file already exists
            file_path = file_path + str(time.time()) # add a timestamp to avoid overwriting existing file
        plt.savefig(file_path + '.png', dpi=500)
    if print_results:
...
def graph_pauli():
    ...
    for i in range(len(actual_zne)): # print labels with the actual extrapolation values under each point
        plt.annotate(str(actual_zne[i][1]), xy=(actual_zne[i][0], height_cutoff - 0.02), xytext=(actual_zne[i][0], height_cutoff - 0.045 - 0.045*actual_zne[i][2]), ha='center', va='bottom', fontsize=10)

    plt.scatter(statevector, raw_error, color=raw_color, label='Unmitigated', s=8, alpha=0.6, marker='o', zorder=1)
    plt.scatter(statevector, rida_error, color=rida_color, label='RIDA', s=8, alpha=0.6, marker='s', zorder=4)
    plt.scatter(statevector, zne_error, color=zne_color, label='Exponential ZNE + TREX', s=8, alpha=0.6, marker='^', zorder=2)
    plt.scatter(statevector, cnot_error, color=cnot_color, label='CNOT-Only Depolarization + Quadratic ZNE', s=8, alpha=0.6, marker='x', zorder=3)
    plt.xlabel('True Expectation', fontsize=16)
    plt.ylabel('Error', fontsize=16)
    plt.xlim(-1,1)
    plt.ylim(0, height_cutoff) # bottom cannot be zero if logscale
    fig.get_layout_engine().set(rect=(0, 0, 1, 0.90)) # add additional space at the top to fit the legend and title
    fig.suptitle(str(nqubits) + ' qubits, ' + error_statement, fontsize=14)
    fig.legend(loc='outside upper center', bbox_to_anchor=(0.5, 0.96), ncol=2, fontsize=10, markerscale=3) # place the legend below the title
    if save_results:
        file_path = 'PAULI GRAPH - ' + str(nqubits) + ' qubits, ' + error_statement
        if os.path.exists(file_path + '.png'): # check if the file already exists
            file_path = file_path + str(time.time()) # add a timestamp to avoid overwriting existing file
        plt.savefig(file_path + '.png', dpi=500)
...
if 'coherent' in file_name: # add coherent error to labels
    error_statement = str(error_multiplier) + 'x coherent error'
else:
    error_statement = str(error_multiplier) + 'x error'
