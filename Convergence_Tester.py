...
shots_per_circuit = 2*10**5 # number of shots used in each depolarization estimation circuit
max_circuits = 100 #50 # largest number of depolarizing estimation circuits tested
pool_size = 1000 #500 # total number of depolarizing estimation circuits to test
nsubsets = 20000 #5000 # total number of random subsets to use for each number of circuits
...
def generate_benchmarks():
    ...
    for j in range(pool_size): # average depolarization magnitude across many circuits
        ...
        rida_depo[j] = 1 - run_circ(ri_circ, shots_per_circuit, measured_qubits)
        cnot_depo_circ = filter_2qbt(circ_t)
        cnot_depo[j] = 1 - run_circ(cnot_depo_circ, shots_per_circuit, measured_qubits)
...
if save_results:
    save_dict = {'nqubits': nqubits, 'error_multiplier': error_multiplier, 'max_circuits': max_circuits,
                 'rida_rmse': rida_rmse, 'cnot_rmse': cnot_rmse, 'shots_per_circuit': shots_per_circuit}
